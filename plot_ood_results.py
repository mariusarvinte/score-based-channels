...
for target_alpha in range(3):
    ...
    our_results = np.mean(our_log[target_alpha, :, our_stop_array[target_alpha]], axis=-1)
    our_results = 10 * np.log10(our_results)

    # L1 results
    l1_dir = "results_l1_saved_channels_seed9999"
    l1_file = l1_dir + "/%s_spacing0.50/l1_results_lmbda3.00e-02.pt" % (target_channel)
    contents = torch.load(l1_file)
    l1_log = contents["oracle_log"].squeeze()
    # Downselect alpha, timestep and average
    l1_results = np.mean(l1_log[target_alpha, :, l1_stop_array[target_alpha] - 1], axis=-1)
...
plt.ylim([-25, 6])
# Save plot
plt.savefig("CDL_C_SNRblind_results.png", bbox_inches="tight", pad_inches=0.05, dpi=300)
...
for target_alpha in range(3):
    ...
    # Downselect alpha, timestep and average
    l1_results = 10 * np.log10(np.min(avg_l1_log[:, np.arange(l1_stop_array[target_alpha] - 1)], axis=-1))
...
# Save-sausage
plt.savefig("CDL_C_SNRknown_results.png", bbox_inches="tight", pad_inches=0.05, dpi=300)
