...
class DCGAN_D(nn.Module):
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0):
        ...
        main.add_module("initial:{0}-{1}:conv".format(nc, ndf), nn.Conv2d(nc, ndf, 4, 2, 1, bias=False))
        ...
        for t in range(n_extra_layers):
            main.add_module(
                "extra-layers-{0}:{1}:conv".format(t, cndf), nn.Conv2d(cndf, cndf, 3, 1, 1, bias=False)
            )
        ...
        main.add_module(
            "final:{0}-{1}:conv".format(cndf, 1), nn.Conv2d(cndf, 1, (4, 16), 1, (0, 0), bias=False)
        )
        self.main = main
...
class DCGAN_G_Ours(nn.Module):
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
        ...
        conv = nn.Sequential()
        # First block
        conv.add_module("up1", nn.Upsample(scale_factor=(2, 2), mode="nearest"))
        conv.add_module("conv1", nn.Conv2d(ngf, ngf, 5, padding=2))
        conv.add_module("bn1", nn.BatchNorm2d(ngf))
        conv.add_module("relu1", nn.ReLU())

        # Second
        conv.add_module("up2", nn.Upsample(scale_factor=(2, 2), mode="nearest"))
        ...
        # Extra layers
        for t in range(n_extra_layers):
            conv.add_module("extra_conv%d" % t, nn.Conv2d(ngf, ngf, 3, 1, 1, bias=False))
            conv.add_module("extra_bn%d" % t, nn.BatchNorm2d(ngf))
            conv.add_module("extra_relu%d" % t, nn.ReLU())
