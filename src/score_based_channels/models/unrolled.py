...
class DnCNN(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=64, nb=17, kernel_size=3, act_mode="BR"):
        """
        # ------------------------------------
        ...
        m_head = B.conv(in_nc, nc, kernel_size=kernel_size, mode="C" + act_mode[-1], bias=bias)
        m_body = [
            B.conv(nc, nc, kernel_size=kernel_size, mode="C" + act_mode, bias=bias) for _ in range(nb - 2)
        ]
        m_tail = B.conv(nc, out_nc, kernel_size=kernel_size, mode="C", bias=bias)
...
class LDAMP(nn.Module):
    def __init__(self, hparams):
        ...
        if hparams.shared_nets:
            if hparams.backbone == "DnCNN":
                update_nets = [DnCNN(in_channels, in_channels, hidden_channels, kernel_size=kernel_size)]
                # Initialize all weights
                update_nets[0].apply(init_weights)
            elif hparams.backbone == "UNet":
                update_nets = [NormUnet(chans=16, num_pools=3)]

        else:
            update_nets = []
            for idx in range(hparams.max_unrolls):
                if hparams.backbone == "DnCNN":
                    local_net = DnCNN(in_channels, in_channels, hidden_channels, kernel_size=kernel_size)
                    # Initialize all weights
    ...
    def forward(self, sample, num_unrolls):
        ...
        # Initial values
        h = 0.0 * torch.randn(y.shape[0], P.shape[-1], y.shape[-1], dtype=y.dtype, device="cuda")
        z = y
        ...
        for unroll_idx in range(num_unrolls):
            ...
            with torch.no_grad():
                ...
                random_dir = torch.randn(r_real.shape, dtype=r_real.dtype, device="cuda")

                # Travel a small amount
                eps = torch.maximum(
                    torch.amax(torch.abs(r), dim=(-1, -2), keepdim=False) * 1e-3, self.safety_min
                )
                if self.logging:
                ...
                div = 1 / eps * torch.mean(random_dir * (h_real_perturbed - h_real), dim=(-1, -2, -3))
                if self.logging:
            ...
            if self.logging:
                r_perturbed_log.append(
                    copy.deepcopy(
                        (r_real_perturbed[:, 0] + 1j * r_real_perturbed[:, 1]).cpu().detach().numpy()
                    )
                )
                h_perturbed_log.append(
                    copy.deepcopy(
                        (h_real_perturbed[:, 0] + 1j * h_real_perturbed[:, 1]).cpu().detach().numpy()
                    )
                )
        ...
        if self.logging:
            return h, h_log, h_perturbed_log, z_log, div_log, r_log, r_perturbed_log, eps_log
        else:
            return h
