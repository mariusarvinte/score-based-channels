...
noise_range = 10 ** (-snr_range / 10.0) * args.antennas[1]
...
# Global results
nmse_log = np.zeros(
    (len(spacing_range), len(alpha_range), len(lmbda_range), len(lr_range), len(snr_range), kept_samples)
)  # Should match data
complete_log = np.zeros(
    (
        len(spacing_range),
        len(alpha_range),
        len(lmbda_range),
...
result_dir = "./results/l1CS_lifted%d/train-%s_test-%s" % (lifting, args.train, args.test)
os.makedirs(result_dir, exist_ok=True)

# Wrap sparsity, steps and spacings
meta_params = itertools.product(spacing_range, alpha_range, lmbda_range, lr_range)
...
for meta_idx, (spacing, alpha, lmbda, lr) in tqdm(enumerate(meta_params)):
    ...
    spacing_idx, alpha_idx, lmbda_idx, lr_idx = np.unravel_index(
        meta_idx, (len(spacing_range), len(alpha_range), len(lmbda_range), len(lr_range))
    )
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=len(val_dataset), shuffle=False, num_workers=0, drop_last=True
    )
    ...
    left_dict = np.conj(ifft(np.eye(val_H[0].shape[0]), n=val_H[0].shape[0] * lifting, norm="ortho"))
    right_dict = ifft(np.eye(val_H[0].shape[1]), n=val_H[0].shape[1] * lifting, norm="ortho").T
    # Lifted shape
    ...
    for snr_idx, local_noise in tqdm(enumerate(noise_range)):
        ...
        val_Y = val_Y + np.sqrt(local_noise) / np.sqrt(2.0) * (
            np.random.normal(size=val_Y.shape) + 1j * np.random.normal(size=val_Y.shape)
        )
        ...
        for sample_idx in tqdm(range(val_Y.shape[0])):
            ...
            array_op = sp.linop.Compose(
                (
                    sp.linop.MatMul((lifted_shape[0], val_H[sample_idx].shape[1]), left_dict),
                    sp.linop.RightMatMul(lifted_shape, right_dict),
                )
            )
            fw_op = sp.linop.Compose(
                (sp.linop.MatMul(val_H[sample_idx].shape, val_P[sample_idx]), array_op)
            )
            ...
            alg = sp.alg.GradientMethod(
                gradf, val_H_hat, lr, proxg=prox_op, max_iter=gd_iter, accelerate=True
            )
            ...
            for step_idx in range(gd_iter):
                ...
                complete_log[spacing_idx, alpha_idx, lmbda_idx, lr_idx, snr_idx, step_idx, sample_idx] = (
            ...
            nmse_log[spacing_idx, alpha_idx, lmbda_idx, lr_idx, snr_idx, sample_idx] = (
                np.sum(np.square(np.abs(est_H - val_H[sample_idx])), axis=(-1, -2))
            ) / np.sum(np.square(np.abs(val_H[sample_idx])), axis=(-1, -2))
...
for alpha_idx, local_alpha in enumerate(alpha_range):
    for snr_idx, local_snr in enumerate(snr_range):
        ...
        print(
            "SNR = %.2f dB, NMSE = %.2f dB using lambda = %.1e and step size = %.1e"
            % (
                local_snr,
                10 * np.log10(best_nmse[alpha_idx, snr_idx]),
                best_lmbda[alpha_idx, snr_idx],
                best_lr[alpha_idx, snr_idx],
            )
        )
...
for alpha_idx, local_alpha in enumerate(alpha_range):
    plt.plot(snr_range, 10 * np.log10(best_nmse[alpha_idx]), linewidth=4, label="Alpha=%.2f" % local_alpha)
plt.grid()
plt.legend()
...
plt.ylabel("NMSE [dB]")
plt.tight_layout()
plt.savefig(os.path.join(result_dir, "results.png"), dpi=300, bbox_inches="tight")
plt.close()
