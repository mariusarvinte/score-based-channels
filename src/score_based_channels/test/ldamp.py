...
parser.add_argument("--snr_range", nargs="+", type=float, default=np.arange(-10, 35, 5))
...
num_channels = 100
nmse_log = np.zeros((len(spacing_range), len(pilot_alpha_range), len(snr_range), num_channels))
# Result directory
result_dir = "./results/ldamp/train-%s_test-%s" % (args.train, args.test)
...
for meta_idx, (spacing, pilot_alpha, snr) in tqdm(enumerate(meta_params)):
    ...
    spacing_idx, pilot_alpha_idx, snr_idx = np.unravel_index(
        meta_idx, (len(spacing_range), len(pilot_alpha_range), len(snr_range))
    )

    # Load model weights
    target_dir = "./models/ldamp-%s/train-%s" % (train_backbone, args.train)
    target_file = os.path.join(target_dir, "model_snr%.2f_alpha%.2f.pt" % (snr, pilot_alpha))
    ...
    val_config.data.noise_std = 10 ** (-val_config.data.train_snr / 20.0) * np.sqrt(
        config.data.image_size[1]
    )

    # Get training dataset for normalization purposes
    if meta_idx == 0:
        train_dataset = Channels(train_seed, config, norm=config.data.norm_channels)
    ...
    for batch_idx, sample in tqdm(enumerate(dataloader)):
        ...
        with torch.no_grad():
            ...
            # Compute NMSE
            nmse_loss = torch.sum(
                torch.square(torch.abs(H_est - sample["H_herm_cplx"])), dim=(-1, -2)
            ) / torch.sum(torch.square(torch.abs(sample["H_herm_cplx"])), dim=(-1, -2))

        # Store NMSE for each channel instance
        nmse_log[spacing_idx, pilot_alpha_idx, snr_idx] = nmse_loss.cpu().detach().numpy()
...
for alpha_idx, local_alpha in enumerate(pilot_alpha_range):
    for snr_idx, local_snr in enumerate(snr_range):
        ...
        # Print result
        print("Learned D-AMP: SNR = %.2f dB, NMSE = %.2f dB" % (local_snr, 10 * np.log10(local_nmse)))
...
for alpha_idx, local_alpha in enumerate(pilot_alpha_range):
    plt.plot(
        snr_range, 10 * np.log10(avg_nmse[0, alpha_idx]), linewidth=4, label="Alpha=%.2f" % local_alpha
    )
plt.grid()
plt.legend()
