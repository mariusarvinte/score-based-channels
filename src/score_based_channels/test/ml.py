...
# Global results
oracle_log = np.zeros(
    (len(spacing_range), len(alpha_range), len(snr_range), kept_samples)
)  # Should match data
result_dir = "results_ml_baseline/model_%s_channel_%s" % (args.model, args.channel)
...
for meta_idx, (spacing, alpha) in tqdm(enumerate(meta_params)):
    ...
    (
        spacing_idx,
        alpha_idx,
    ) = np.unravel_index(meta_idx, (len(spacing_range), len(alpha_range)))
    ...
    for snr_idx, local_noise in tqdm(enumerate(noise_range)):
        ...
        for sample_idx in tqdm(range(val_Y.shape[0])):
            # Normal equation
            normal_P = np.matmul(val_P[sample_idx].T.conj(), val_P[sample_idx]) + local_noise * np.eye(
                val_P[sample_idx].shape[-1]
            )
...
torch.save(
    ...
    result_dir + "/results_Nt%d_Nr%d.pt" % (args.antennas[1], args.antennas[0]),
)
