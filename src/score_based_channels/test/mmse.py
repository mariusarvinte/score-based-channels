...
# Target weights
deep_mimo_set = ["DeepMIMO_outdoor", "DeepMIMO_indoor_1", "DeepMIMO_indoor_3_nlos"]
if args.model == "CDL-D":
...
config.sampling.sigma = 0.0  # Nothing here
config.purpose = "train"

# More sigmas
config.model.num_classes = args.num_classes
# config.model.sigma_begin = 2.
config.model.sigma_rate = (config.model.sigma_end / config.model.sigma_begin) ** (
...
config.sampling.actual_steps = config.sampling.steps_each * np.ones(config.model.num_classes, dtype=int)
# Total overall steps
...
# Global results
oracle_log = np.zeros(
    (len(spacing_range), len(pilot_alpha_range), len(snr_range), total_steps, kept_samples, mmse_avg)
)  # Should match data
...
saved_H = np.zeros(
    (len(spacing_range), len(pilot_alpha_range), len(snr_range), kept_samples, mmse_avg, 64, 16),
    dtype=np.complex64,
)
...
for meta_idx, (spacing, pilot_alpha) in tqdm(enumerate(meta_params)):
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=kept_samples, shuffle=False, num_workers=0, drop_last=True
    )
    ...
    _, val_P, _ = val_sample["H"].cuda(), val_sample["P"].cuda(), val_sample["Y"].cuda()
    # Transpose pilots
    ...
    for snr_idx, local_noise in tqdm(enumerate(noise_range)):
        ...
        local_Y = local_Y + np.sqrt(local_noise) * torch.randn_like(local_Y)

        # For each sample, add exact copies to collections
        for sample_idx in range(len(val_H)):
            global_Y.append(torch.tile(local_Y[sample_idx][None, ...].clone(), (mmse_avg, 1, 1)))
            global_P.append(torch.tile(val_P[sample_idx][None, ...].clone(), (mmse_avg, 1, 1)))
            global_H.append(torch.tile(val_H[sample_idx][None, ...].clone(), (mmse_avg, 1, 1)))
        ...
        if args.start_point == "Noise":
            ...
            current = torch.matmul(torch.conj(torch.transpose(global_P, -1, -2)), global_Y)
        elif args.start_point == "LS":
            current = torch.linalg.lstsq(global_P.cpu(), global_Y.cpu(), driver="gelsd").cuda()
        ...
        with torch.no_grad():
            for step_idx in tqdm(range(val_config.model.num_classes)):
                ...
                for inner_idx in range(config.sampling.actual_steps[step_idx]):
                    ...
                    # Compute alpha
                    alpha = val_config.model.step_size * (current_sigma / val_config.model.sigma_end) ** 2
                    ...
                    current = (
                        current
                        + alpha
                        * (score - args.dc_boost * meas_grad / (local_noise / 2.0 + current_sigma**2))
                        + grad_noise
                    )

                    # Store loss
                    oracle_log[spacing_idx, pilot_alpha_idx, snr_idx, trailing_idx] = np.reshape(
                        (
                            torch.sum(torch.square(torch.abs(current - oracle)), dim=(-1, -2))
                            / torch.sum(torch.square(torch.abs(oracle)), dim=(-1, -2))
                        )
                        .view(len(snr_range), -1)
                        .cpu()
                        .numpy(),
                ...
                if mark_break:
                    ...
                    saved_H[spacing_idx, pilot_alpha_idx, snr_idx] = copy.deepcopy(
...
torch.save(save_dict, result_dir + "/model_%s_channel_%s.pt" % (args.model, args.channel))
