...
nmse_log = np.zeros(
    (
        len(spacing_range),
        len(pilot_alpha_range),
        len(snr_range),
        int(config.model.num_classes * config.sampling.steps_each),
        num_channels,
    )
)
result_dir = "./results/score/train-%s_test-%s" % (args.train, args.test)
...
for meta_idx, (spacing, pilot_alpha) in tqdm(enumerate(meta_params)):
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=num_channels, shuffle=False, num_workers=0, drop_last=True
    )
...
for alpha_idx, local_alpha in enumerate(pilot_alpha_range):
    plt.plot(
        snr_range, 10 * np.log10(best_nmse[0, alpha_idx]), linewidth=4, label="Alpha=%.2f" % local_alpha
    )
plt.grid()
plt.legend()
