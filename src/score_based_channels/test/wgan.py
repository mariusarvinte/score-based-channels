...
parser.add_argument("--l2lam_range", nargs="+", type=float, default=[1e-1, 3e-1, 1.0, 3.0])
parser.add_argument("--lr_range", nargs="+", type=float, default=[0.03, 0.01, 0.003, 0.001])
parser.add_argument("--alpha_range", nargs="+", type=float, default=[0.6, 0.8, 1.0])
...
dataloader = torch.utils.data.DataLoader(
    dataset, batch_size=config.batchSize, shuffle=True, num_workers=2 if os.name == "posix" else 0
)
...
# Logs
oracle_log = np.zeros(
    (len(l2lam_range), len(lr_range), len(pilot_alpha_range), len(snr_range), total_steps, kept_samples)
)  # Last one is num channels
...
for meta_idx, (l2_lam, lr, pilot_alpha) in tqdm(enumerate(meta_params)):
    ...
    val_H, val_P, _ = val_sample["H"].cuda(), val_sample["P"].cuda(), val_sample["Y"].cuda()
    # Complexity
    ...
    for snr_idx, local_noise in tqdm(enumerate(noise_range)):
        ...
        val_Y = val_Y + np.sqrt(local_noise) / np.sqrt(2.0) * torch.randn_like(val_Y)
        oracle = val_H
        ...
        latent_z = torch.tensor(init_z, dtype=torch.float32, requires_grad=True, device="cuda:0")
        ...
        for step_idx in tqdm(range(total_steps)):
            ...
            meas_loss = torch.sum(torch.square(torch.abs(gen_meas - val_Y)), axis=(-1, -2))
            reg_loss = torch.sum(torch.square(torch.abs(latent_z)), axis=(-1, -2, -3))
            ...
            with torch.no_grad():
                oracle_log[l2_idx, lr_idx, alpha_idx, snr_idx, step_idx] = (
                    (
                        torch.sum(torch.square(torch.abs(gen_channels - oracle)), dim=(-1, -2))
            ...
            meas_log[l2_idx, lr_idx, alpha_idx, snr_idx, step_idx] = meas_loss.detach().cpu().numpy()
            reg_log[l2_idx, lr_idx, alpha_idx, snr_idx, step_idx] = reg_loss.detach().cpu().numpy()
...
torch.save(
    ...
    result_dir + "/wgan_results_model%s_channel%s_DETAILED.pt" % (args.model, args.channel),
)
