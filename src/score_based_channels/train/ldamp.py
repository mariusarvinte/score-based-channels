...
for noise_idx, train_snr in enumerate(args.snr_range):
    ...
    config.data.noise_std = 10 ** (-config.data.train_snr / 20.0) * np.sqrt(config.data.image_size[1])
    ...
    config.input_dim = np.prod(config.data.image_size) * config.data.channels
    ...
    num_params = np.sum([np.prod(p.shape) for p in model.parameters() if p.requires_grad])
    print("Model has %d trainable parameters" % num_params)

    # Get optimizer and scheduler
    optimizer = torch.optim.Adam(model.parameters(), config.optim.lr)
    scheduler = StepLR(optimizer, config.training.decay_epochs, gamma=config.training.decay_gamma)

    # Result directory
    local_dir = "./models/ldamp-%s/train-%s" % (config.model.backbone, args.train)
    ...
    for epoch_idx in tqdm(range(config.training.n_epochs)):
        ...
        for batch_idx, sample in tqdm(enumerate(dataloader)):
            ...
            loss = torch.mean(
                torch.sum(torch.square(torch.abs(H_est - sample["H_herm_cplx"])), dim=(-1, -2))
            )
            ...
            # Verbose
            print(
                "Epoch %d, Step %d, Train Loss %.3f, Train SNR %.2f dB, Train NMSE %.3f dB"
    ...
    torch.save(
        ...
        os.path.join(local_dir, "model_snr%.2f_alpha%.2f.pt" % (config.data.train_snr[0], args.alpha)),
    )
