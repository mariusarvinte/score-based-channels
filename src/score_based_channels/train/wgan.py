...
if len(config.data.spacing_list) == 1:
    model_dir = "./models/wgan_%s_%.2f" % (config.data.channel, config.data.spacing_list[0])
else:
    model_dir = "./models/wgan_%s_min%.2f_max%.2f" % (
        config.data.channel,
        np.min(config.data.spacing_list),
        np.max(config.data.spacing_list),
    )
