...
parser.add_argument("--alpha_step_range", nargs="+", type=float, default=[3e-11, 6e-11, 1e-10, 3e-10])
parser.add_argument("--beta_noise_range", nargs="+", type=float, default=[0.1, 0.01, 0.001])
...
noise_range = 10 ** (-snr_range / 10.0) * config.data.image_size[1]

# Global results
nmse_log = np.zeros(
    (
        len(alpha_step_range),
        len(beta_noise_range),
        len(snr_range),
...
for meta_idx, (alpha_step, beta_noise) in tqdm(enumerate(meta_params)):
    # Unwrap indices
    alpha_idx, beta_idx = np.unravel_index(meta_idx, (len(alpha_step_range), len(beta_noise_range)))
    ...
    val_config.data.num_pilots = int(np.floor(config.data.image_size[1] * args.pilot_alpha))
    ...
    for snr_idx, local_noise in tqdm(enumerate(noise_range)):
        ...
        for step_idx in tqdm(range(val_config.model.num_classes)):
            ...
            for inner_idx in range(val_config.sampling.steps_each):
                ...
                # Sample noise
                grad_noise = np.sqrt(2 * alpha * beta_noise) * torch.randn_like(current)

                # Apply update
                current = (
                    current
                    + alpha * (score - meas_grad / (local_noise / 2.0 + current_sigma**2))
                    + grad_noise
                )
...
for alpha_idx, local_alpha in enumerate(alpha_step_range):
    for beta_idx, local_beta in enumerate(beta_noise_range):
        plt.plot(
            snr_range,
            10 * np.log10(best_nmse[alpha_idx, beta_idx]),
            linewidth=4,
            label="Alpha=%.2e, Beta=%.2e" % (local_alpha, local_beta),
        )
plt.grid()
plt.legend()
...
plt.ylabel("NMSE [dB]")
plt.tight_layout()
plt.savefig(os.path.join(result_dir, "%s-hyperparameters.png" % args.channel), dpi=300, bbox_inches="tight")
plt.close()
